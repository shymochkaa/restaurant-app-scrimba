...
document.addEventListener("click", (e) => {
  ...
  button &&
    button.dataset.buttonIndex &&
    handleAddToOrderBtnClick(button.dataset.buttonIndex);
});
...
document.addEventListener(
  "click",
  (e) =>
    e.target.dataset.removeButton &&
    deleteItemFromOrder(e.target.dataset.removeButton)
);

const deleteItemFromOrder = (orderItemIndex) => {
  orderArr.splice(orderItemIndex, 1);
  const orderToRemove = document.querySelector(
    `[data-remove-button="${orderItemIndex}"]`
  );
  orderToRemove.parentElement.remove();
  const orderItems = document.querySelectorAll(
    ".order-item button[data-remove-button]"
  );
  orderItems.forEach((button, newIndex) =>
    button.setAttribute("data-remove-button", newIndex)
  );
  updateTotal(orderArr);
  displayOrders();
};
...
const displayOrders = () =>
  orderDetails.classList.toggle("hidden", orderArr.length === 0);
